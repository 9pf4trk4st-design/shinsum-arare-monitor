...
def parse_slit_alerts(page):
    ...
    try:
        ...
        for i in range(count):
            ...
            try:
                txt = row.inner_text(timeout=1200)
            except Exception:
                continue
            ...
            slit_value = m.group(1)
            boost_value = float(m.group(2))
...
def notify_slit_alert(
    ...
    スリットアラート専用ntfy通知。
    """
    if venue not in SLIT_ALERT_VENUES:
        return
    ...
    for x in alerts:
        ...
        if key in SLIT_ALERT_SENT:
            continue

        SLIT_ALERT_SENT.add(key)
        new_alerts.append(x)
    ...
    print(
        f"スリットアラート通知送信: "
        f"{venue} / {race} / "
        f"{[x['boat'] for x in new_alerts]}号艇",
        flush=True
    )


def inspect_slit_alert(page):
    """
    現在の詳細ページでスリットアラートだけ確認する。
    既存の最終補正1着率ロジックとは完全に独立。
    """
    try:
    ...
    venue = actual_venue(body_text)
    race = actual_race(body_text)
    ...
    # 既存監視と同様、締切15分以内だけ通知
    if not within15(body_text):
        return

    d = deadline(body_text)
...
def inspect(page):
    ...
    if collapse:
        collapse["current_diffs"] = current_diffs
        return {
            "venue": v,
            "race": r,
            "deadline": d,
    ...
    if escape:
        escape["current_diffs"] = current_diffs
        return {
            "venue": v,
            "race": r,
            "deadline": d,
    ...
    if a:
        ...
        if classification:
            classification["current_diffs"] = current_diffs
            return {
                "venue": v,
                "race": r,
                "deadline": d,
...
def cycle(page, initial=False):
    ...
    for u in links[:100]:
        try:
            ...
            # V35: スリットアラート専用。
            # 旧シンsum通知（逃げ強化・逃げ崩れ・1号艇有利など）は一切実行しない。
            inspect_slit_alert(page)
