#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Musigny FX Paper Bot v3 - PAPER ONLY
# closer TP / review re-evaluation / max leverage 10x / detailed entry reasons
...
def notify(t):
    print(t,flush=True)
    if NTFY_TOPIC:requests.post(f'https://ntfy.sh/{NTFY_TOPIC}',data=t.encode('utf-8'),headers={'Title':'Musigny FX Paper Bot v3'},timeout=15).raise_for_status()
...
def make_pos(s,p,tickers):
    ...
    s['pending']=None
    reasons=' / '.join(p.get('reasons',[])[:8]) or 'NONE'
    counter=' / '.join(p.get('counter',[])[:5]) or 'NONE'
    return (
        f'FX PAPER OPEN {p["symbol"]} {p["side"]}\n'
        f'Confidence {p.get("confidence",0)}/100 | LONG={p.get("long","?")} SHORT={p.get("short","?")}\n'
        f'Entry {e:.5f}\nSTOP {p["stop"]:.5f}\n'
        f'TP1 {p["tp1"]:.5f} TP2 {p["tp2"]:.5f} TP3 {p["tp3"]:.5f}\n'
        f'ENTRY REASONS: {reasons}\n'
        f'COUNTER: {counter}\n'
        f'Max leverage {MAX_LEVERAGE:.1f}x | Risk {RISK_PCT*100:.2f}%'
    )
...
def manage_position(s,tickers,analyses):
    ...
    if not p['tp1_done'] and tp(p['tp1']):
        ...
        msgs.append(f'FX PAPER TP1 {p["symbol"]} 50% PnL {pnl:+,.0f}')
    ...
    if not p['tp2_done'] and tp(p['tp2']):
        ...
        msgs.append(f'FX PAPER TP2 {p["symbol"]} 30% PnL {pnl:+,.0f}')
