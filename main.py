...
MAX_LEVERAGE = float(os.getenv("MAX_LEVERAGE", "10"))  # ペーパー上の最大レバレッジ
MAX_DAILY_LOSS_PCT = float(os.getenv("MAX_DAILY_LOSS_PCT", "0.03"))
MAX_CONSECUTIVE_LOSSES = int(os.getenv("MAX_CONSECUTIVE_LOSSES", "3"))
MAX_OPEN_POSITIONS = int(os.getenv("MAX_OPEN_POSITIONS", "2"))
TP1_PCT, TP2_PCT, TP3_PCT = 0.30, 0.40, 0.30
PENDING_EXPIRE_HOURS = float(os.getenv("PENDING_EXPIRE_HOURS", "1.5"))
...
def rci_snapshot(ind):
    r=ind.iloc[-1]
    return {
        "r8": float(r["rci8"]) if pd.notna(r["rci8"]) else None,
        "r25": float(r["rci25"]) if pd.notna(r["rci25"]) else None,
        "r47": float(r["rci47"]) if pd.notna(r["rci47"]) else None,
        "d8": rci_dir(ind,8),
        "d25": rci_dir(ind,25),
        "d47": rci_dir(ind,47),
    }
...
def analyze(symbol,frames):
    ...
    p=float(c15.close)
    rci_allow_long,rci_allow_short,rci_long_reasons,rci_short_reasons=multi_tf_rci_gate(W,D,H4,H1,M15,M5)
    ...
    allow_long=rci_allow_long
    allow_short=rci_allow_short
    rl += rci_long_reasons
...
def make_position(state,p,tickers):
    ...
    qty=min(
        ...
        (state["paper_balance"]*MAX_LEVERAGE)/mid
    )
    ...
    return (
        ...
        f"利確②: {p['tp2']:,.4f}\n"
        f"利確③: {p['tp3']:,.4f}\n"
        f"最大レバレッジ: {MAX_LEVERAGE:.1f}倍（ペーパー）\n"
        f"1回の許容リスク: {RISK_PCT*100:.2f}%\n"
        f"※損益通知・残高は想定コスト差引後の純利益ベース"
    )
...
def manage_pending(state,analyses,tickers):
    ...
    if now_utc()-datetime.fromisoformat(p["created_at"])>timedelta(hours=PENDING_EXPIRE_HOURS):
        state["pending"]=None
        return f"⌛ {p['symbol']} {jp_side(p['side'])}候補が失効しました"

    a=next((x for x in analyses if x.symbol==p["symbol"]),None)
    if not a:
        return None

    # 約定直前に最新の全時間足RCI判定を必ず再確認
    if a.side!=p["side"] or a.confidence<ENTRY_THRESHOLD:
        state["pending"]=None
        return (
            f"❌ {p['symbol']} {jp_side(p['side'])}候補を取消しました\n"
            f"約定直前の再判定: {jp_side(a.side)} / 信頼度 {a.confidence}"
        )

    if p["side"]=="LONG" and a.candle_low<=p["stop"]:
        state["pending"]=None
        return f"❌ {p['symbol']} ロング候補を取消しました"

    if p["side"]=="SHORT" and a.candle_high>=p["stop"]:
        state["pending"]=None
        return f"❌ {p['symbol']} ショート候補を取消しました"

    if touch_zone(p,a.candle_high,a.candle_low):
        msg=make_position(state,a,tickers)
        state["pending"]=None
        return msg
    return None
...
def manage_positions(state,analyses,tickers):
    ...
    for pos in positions:
        ...
        if stop_hit(pos["stop"]):
            msgs.append(close_remaining_multi(state,pos,price,"STOP_END","🛑 仮想損切り")); closed=True
            ...
            msgs.append(close_remaining_multi(state,pos,price,"TRAIL_END","📉 仮想追従決済")); closed=True
        else:
            ...
            if tp_hit(pos["tp3"]):
                msgs.append(close_remaining_multi(state,pos,price,"TP3_END","🏁 仮想最終利確")); closed=True
...
def notify(t):
    ...
    if not NTFY_TOPIC:
        return
    headers={
        "Title":"Musigny Crypto BOT v11.2",
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code==429:
                wait=10*(attempt+1)
                print(f"[NTFY-WARN] 429 Too Many Requests; retry in {wait}s",flush=True)
                time.sleep(wait)
                continue
            r.raise_for_status()
            return
            ...
            if attempt<2:
                wait=5*(attempt+1)
                print(f"[NTFY-WARN] {e}; retry in {wait}s",flush=True)
                time.sleep(wait)
            else:
                print(f"[NTFY-SKIP] notification failed; bot continues: {e}",flush=True)
                return
